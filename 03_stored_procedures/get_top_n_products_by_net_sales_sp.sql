...
-- Purpose: Return top N products by net sales for a given
...
--                         adds dim_product JOIN to carry
--                         product name through the pipeline
...
-- Output: Top N products ranked by net sales (in millions)
-- Difference from get_top_customers_by_net_sales:
--   Adds dim_product JOIN to resolve product name
--   Groups by product instead of customer
...
BEGIN
    ...
    --          dim_product joined here to carry product name
    -- --------------------------------------------------------
    g_price AS (
        SELECT 
            dc.market,
    ...
    -- --------------------------------------------------------
    -- OUTPUT: Top N products by net sales (in millions)
    -- --------------------------------------------------------
    SELECT 
        product, 
